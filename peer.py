...
class RPCThreading(ThreadingMixIn, SimpleXMLRPCServer):  #I have literally no idea what this does, except work
    def __init__(self, addr, requestHandler=SimpleXMLRPCRequestHandler, logRequests=True, allow_none=False,
                 encoding=None, bind_and_activate=True, artLatency=.0):
...
class DHTransport(xmlrpc.client.Transport):  #xmlrpc-client transport support ADH, and pools connections
    ...
    def make_connection(self, host):
        ...
        if not (self.host == host):
            connections = []
            self.host = host
        viables = [connection for connection in self.connections if
                   connection._HTTPConnection__state == 'Idle']  #Python is so slutty
        if len(viables) == 0:
...
class Peer:
    def __init__(self, name, IP, port, peerLimit, artLatency=.0):
        ...
        self.peerSet = set(
            [self.myString])  # adding ourselves to peerSet. design choice. to avoid a lot of bookkeeping.
        ...
        self.pool = ThreadPool(processes=int(4))
    ...
    def neighbouriseMe(self):
        potentials = [peer for peer in self.peerSet if
                      (not peer in self.neighbourSet) and (not strName(peer) == self.name)]
        if (len(potentials) != 0):
    ...
    def handleWalkers(self, key, k, searchId, TTL, originalSourcePeer, sourcePeer):
        ...
        if TTL % 4 == 0:  # only check each 4th step
            try:
                ...
                print(
                    self.name + ": k-walker check for search relevancy failed at " + originalSourcePeer + ". is dead.")
        ...
        if noOfWalkersForEachNeighbour > 0:
            for peer in eligibleNeighbours:
                try:
                    self.forwardWalker(peer, key, noOfWalkersForEachNeighbour, searchId, TTL - 1, originalSourcePeer,
                                       self.myString)
    ...
    def forwardWalker(self, peer, key, k, searchId, TTL, originalSourcePeer, sourcePeer):
        ...
        walkerThread = threading.Thread(target=peerProxy.handleWalkers,
                                        args=(key, k, searchId, TTL, originalSourcePeer, sourcePeer))
        walkerThread.start()
    ...
    def getMessagesPerPeer(self):
        return {strName(peer) + "(" + strLimit(peer) + ")": self.makeProxy(strAddress(peer)).getMessagesPassed() for
                peer in self.peerSet}
    ...
    def addFriend(self, name, keyFile):
        try:
            ...
            cipher = PKCS1_OAEP.new(
                publickey)  #Note PKCS1_OAEP is better known as RSAES-OAEP, and is the 'safe' way to do encryption with RSA
    ...
    def receiveMessage(self, message, sig, proof):  #Note: message is an XMLRPC binary data wrapper
        ...
        if self.cipher is not None:  # No reason to try to snoop without a secret
            try:  #Only recipient can decode data
                ...
                print(
                    'Received Message from ' + self.checkSender(decryptedMessage, sig) + ': ' + decryptedMessage.decode(
                        'utf-8'))  #Get binary data from XMLRPC wrapper, decrypt it, and decode it from UTF-8 from
        ...
        neighbourResults = self.pool.map(self.forwardMessage,
                                         [(peer, message, sig, proof) for peer in self.neighbourSet])
        ...
        if self.cipher is not None:  # No reason to try to snoop without a secret
            try:  #Only recipient can decode data
                ...
                if not (mhash in self.messagesSet):  #We might end up here a lot
                    ...
                    print('Received Message from ' + self.checkSender(decryptedMessage,
                                                                      sig) + ': ' + unnoncedMessage.decode(
                        'utf-8'))  #Get binary data from XMLRPC wrapper, decrypt it, and decode it from UTF-8 from
